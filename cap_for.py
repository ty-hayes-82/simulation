...
tableau_filter_1 = 'gm_region_name'
tableau_parameter_1 = 'gm_region_parameter'
tableau_filter_list_1 = ['Northeast','Southeast','North Central','South Central','Northwest','Southwest']
...
for typ in file_type_list:
    ...
    for line in tableau_view_list_1:
        outF.write('tabcmd get "' + tb_wb_path + '/_' + line + '.' + typ + '?:refresh=yes' + part2 + line + part3)
        outF.write("\n")
    for line in tableau_filter_list_1:
        outF.write(part1 + '&' + tableau_filter_1 + '=' + urllib.parse.quote(line, safe='&=').replace('%', '%%') + '&' + tableau_parameter_1 + '=' + urllib.parse.quote(line, safe='&=').replace('%', '%%') + part2 + line + part3)
        outF.write("\n")
...
for line in tableau_filter_list_1:
    for f in os.listdir():
        if f.startswith(line):
            if f.endswith(line + ".png"):
                image_cid = make_msgid()
                msgText += '<a href="' + tb_hyperlink_path + '/BalanceMapEndofDay?:&' + tableau_filter_1 + '=' + urllib.parse.quote(line)  + '"><img src="cid:{image_cid}"><br>'.format(image_cid=image_cid[1:-1])
                with open(f, 'rb') as f:
                    msgImage = MIMEImage(f.read())
                msgImage.add_header('Content-ID', image_cid)
                msg.attach(msgImage)
