...
export default function ShortcutsView() {
  ...
  const [, setBbox] = React.useState<[number, number, number, number] | null>(null);

  const nodesUrl = React.useMemo(() => {
    if (!selectedCourse) return null;
    return `/${selectedCourse.id}/holes_connected.geojson`;
  }, [selectedCourse]);

  const courseUrl = React.useMemo(() => {
    if (!selectedCourse) return null;
    return `/${selectedCourse.id}/course_polygon.geojson`;
  }, [selectedCourse]);
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', position: 'relative' }}>
      ...
      >
        ...
        {courseUrl && <Source id="course" type="geojson" data={courseUrl}>
